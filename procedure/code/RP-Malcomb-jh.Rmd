...
packages = c("downloader","haven","stars","dplyr","sf","rdhs", "classInt", "readr", "ggplot2", "here", "s2", "knitr")
...
```{r comparing discrete chloropleth maps}
...
write_sf(fig4compare, here("data", "derived", "public", "fig4Comparisons.shp"))
```
```{r comparing continuous Rasters}
orfig5vect = 
  read_sf(here("data", "derived", "public", "georeferencing_vulnerability_grid.shp"))
# load original figure 5 data

orfig5rast = st_rasterize(orfig5vect["rmean"], template=ta_final)
#column is labeled rmean because of human error, it does measure the average blue values
...
write_sf(fig5comppts, here("data", "derived", "public", "fig5Comparisons.shp"))
...
# Spearman's Rho correlation test
```
```{r adaptive capacity difference map}
ac_difference = ggplot() +
  geom_sf(data = ea,
          aes(fill = EA),
          color = NA) +
  geom_sf(
  ...
  scale_x_continuous(breaks = c(33,34,35,36)) +
  labs(title = "Difference in Adaptive Capacity Maps") +
  theme_minimal() +
  theme(legend.title = element_blank())
